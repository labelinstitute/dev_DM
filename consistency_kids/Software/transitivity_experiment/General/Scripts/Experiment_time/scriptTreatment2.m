function [behavioralSecs,behavioralKey,behavioralTallyTotal,behavioralTallyYes,behavioralTallyIndf,behavioralTallyNo,catchMostOption,catchLeastOption] = scriptTreatment2( item, flipLR, input, treat2Order, setTreatment2, Treatment2, treatmentStructure, w )
...
for i = 1:21;
    index = flipLR(i);
    buttonDown = 0;
    renderTreat2(item, Treatment2((index*index + 1),2,(treat2Order(i))),... 
        Treatment2((index+2-index*2),2,(treat2Order(i))), w, 0, buttonDown);
    [VBLTimestamp StimulusOnsetTime FlipTimestamp] = Screen('Flip',w);
    WaitSecs(0.75); %should be 0.75
    renderTreat2(item, Treatment2((index*index + 1),2,(treat2Order(i))),...
        Treatment2((index+2-index*2),2,(treat2Order(i))), w, 1, buttonDown);
    Screen('Flip',w);
    [behavioralSecs, behavioralKey] = touchResponse(w, input, StimulusOnsetTime);
    if behavioralKey == 'f'
        buttonDown = 1;
        if index == 0
            tally(treatmentStructure(treat2Order(i),1)) = tally(treatmentStructure(treat2Order(i),1)) + 1; %what this says: if we have not flipped the left-right order of the options, and the subject has pressed the left button (selected left option), then add 1 to the tally of that option by selecting row corresponding to trial in treatmentStructure.mat and adding 1 to tally of 1st column option
            yesTally(treatmentStructure(treat2Order(i),1)) = yesTally(treatmentStructure(treat2Order(i),1)) + 1;
            noTally(treatmentStructure(treat2Order(i),2)) = noTally(treatmentStructure(treat2Order(i),2)) + 1;
        elseif index == 1
            tally(treatmentStructure(treat2Order(i),2)) = tally(treatmentStructure(treat2Order(i),2)) + 1;
            yesTally(treatmentStructure(treat2Order(i),2)) = yesTally(treatmentStructure(treat2Order(i),2)) + 1;
            noTally(treatmentStructure(treat2Order(i),1)) = noTally(treatmentStructure(treat2Order(i),1)) + 1;
        end
    elseif behavioralKey == 'b'
        buttonDown = 2;
        if index == 0
            tally(treatmentStructure(treat2Order(i),1)) = tally(treatmentStructure(treat2Order(i),1)) + 0.5; 
            tally(treatmentStructure(treat2Order(i),2)) = tally(treatmentStructure(treat2Order(i),2)) + 0.5; 
            indfTally(treatmentStructure(treat2Order(i),1)) = indfTally(treatmentStructure(treat2Order(i),1)) + 1;
            indfTally(treatmentStructure(treat2Order(i),2)) = indfTally(treatmentStructure(treat2Order(i),2)) + 1;
        elseif index == 1
            tally(treatmentStructure(treat2Order(i),1)) = tally(treatmentStructure(treat2Order(i),1)) + 0.5; 
            tally(treatmentStructure(treat2Order(i),2)) = tally(treatmentStructure(treat2Order(i),2)) + 0.5; 
            indfTally(treatmentStructure(treat2Order(i),1)) = indfTally(treatmentStructure(treat2Order(i),1)) + 1;
    ...
    elseif behavioralKey == 'j'
        ...
        if index == 0
            ...
            noTally(treatmentStructure(treat2Order(i),1)) = noTally(treatmentStructure(treat2Order(i),1)) + 1;
        elseif index == 1
            tally(treatmentStructure(treat2Order(i),1)) = tally(treatmentStructure(treat2Order(i),1)) + 1; 
            yesTally(treatmentStructure(treat2Order(i),1)) = yesTally(treatmentStructure(treat2Order(i),1)) + 1;
            noTally(treatmentStructure(treat2Order(i),2)) = noTally(treatmentStructure(treat2Order(i),2)) + 1;
        end
    end
...
renderTreat2(item,setTreatment2(firstOption,2),setTreatment2(secondOption,2),w,0,buttonDown);
[VBLTimestamp StimulusOnsetTime FlipTimestamp] = Screen('Flip',w);
WaitSecs(0.75); %should be 0.75
renderTreat2(item,setTreatment2(firstOption,2),setTreatment2(secondOption,2),w,1,buttonDown);
Screen('Flip',w);
[behavioralSecs, behavioralKey] = touchResponse(w, input, StimulusOnsetTime);
if behavioralKey == 'f'
    buttonDown = 1;
